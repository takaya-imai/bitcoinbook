...
Bitcoinトランザクションの支払いには受取人の公開鍵が表示されており、この公開鍵は _Bitcoinアドレス_ と呼ばれるデジタルフィンガープリントの形で表示されます。このBitcoinアドレスは小切手にある受取人名と同じようにも使われます。ほとんどの場合、Bitcoinアドレスは公開鍵から生成され、１つの公開鍵に対応しています。しかし、全てのBitcoinアドレスが公開鍵を表現しているわけではありません。この後の章でも見る通り、これらはscriptのような形で他の受取人も表現できます。この方法では、Bitcoinアドレスは資金の受取人を要約しており、誰と取引をするか ????
...
|種類| Version prefix (１６進数)| Base58変換したものの接頭語
...
| Bitcoinアドレス | 0x00 | 1 ^M
| Pay-to-Script-Hashアドレス | 0x05 | 3 ^M
| Bitcoin Testnetアドレス | 0x6F | m または n ^M
| 秘密鍵 WIF |  0x80 | 5, K または L ^M
| BIP38 暗号化秘密鍵 | 0x0142 | 6P ^M
| BIP32 拡張公開鍵 | 0x0488B21E | xpub  ^M
|=======
...
<<table_4-5>>はentropyデータのサイズとmnemonic codeの単語数の関係を示しています。

[[table_4-5]]
.Mnemonic codes: entropy and word length
.Mnemonic codes: エントロピーと単語数
[options="header"]
|=======
...
| *エントロピー入力値 (128 bits)*| 0c1e24e5917779d297e14d45f14e1a1a^M
| *Mnemonic (単語数12)* | army van defense carry jealous true garbage claim echo media make crunch^M
| *シード  (512 bits)* | 3338a6d2ee71c7f28eb5b882159634cd46a898463e9d2d0980f8e80dfbba5b0fa0291e5fb88^M
8a599b44b93187be6ee3ab5fd3ead7dd646341b2cdb8d08d13bf7^M
|=======
...
ルートシードはHMAC-SHA512アルゴリズムに対するインプットであり、結果として出力されたハッシュは _マスター秘密鍵_ (m)と _マスターチェーンコード_ を生成することに使われます。マスター秘密鍵(m)は対応したマスター公開鍵(M)を生成し、このときこの章の最初に見た標準的な楕円曲線上のスカラー倍算プロセス +m * G+ が使われています。このチェーンコードは親キーから子キーを生成するプロセスの中でエントロピーを導入するために使われます。????
...
拡張キーは秘密鍵または公開鍵、チェーンコードで構成されています。拡張キーは子供を生成し、ツリー構造の中に子供自身のブランチを作り出していくことができます。拡張キーを共有することで、ブランチ全体にアクセスすることができます。????
==== 
...
P2SHは必然的にマルチシグネチャ標準トランザクションとは違うものになります。
P2SHアドレスは _ほぼ常に_ マルチシグネチャscriptを表しますが、
====
