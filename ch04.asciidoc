...
bitcoinトランザクションの支払いには受取人の公開鍵が表示されており、この公開鍵は _bitcoinアドレス_ と呼ばれるデジタル指紋の形で表示されます。このbitcoinアドレスは小切手にある受取人名と同じようにも使われます。ほとんどの場合、bitcoinアドレスは公開鍵に紐づいています。しかし、bitcoinアドレスだけが公開鍵ではありません。この後の章でも見る通り、scriptのような形でも受取人を表現できます。この方法では、bitcinアドレスは資金の受取人を ????
