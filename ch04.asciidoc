...
加法が定義されたので、加法を拡張する標準的な方法に沿って乗法を定義できます。
楕円曲線上の点Pに対して、もしkが整数だとすると、kP = P + P + P + ... + P (k回)。この場合、紛らわしいことにkがときどきべき指数と呼ばれるのです。(((range="endofrange", startref="ix_ch04-asciidoc5")))(((range="endofrange", startref="ix_ch04-asciidoc4")))
