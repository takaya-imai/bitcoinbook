...
自然は、分散システムは快活であり創発的複雑さや信じられないような知的素養　中心的な権威や階層、複雑な部品を必要とすることなく
...
Bitcoinは高度に知的に発達した分散された信用ネットワークです。この信用ネットワークは無数の経済的プロセスをサポートできます。
まだBitcoinネットワークのそれぞれのノードはいくつかの数学的ルールに従っています。
多くのノード間の相互作用がどんな洗練された    個々のノードに対する固有の複雑さや信用なしに
アリのコロニーのように、Bitcoinネットワークは単純なルールに従う複数の単純なノードによって生じる快活的ネットワークです。
...
全てのコードスニペットは、言語に対応したコンパイラとインタプリタを最小のインストール(依存関係が少ない)とほとんどのオペレーティングシステム上で複製することができ、
