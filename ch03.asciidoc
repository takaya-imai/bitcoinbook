...
あなたは"Satoshi client"とも呼ばれているbitcoin coreのいろいろな実装をbitcoin.orgからダウンロードすることができます。
このクライアントはbitcoinの仕組みのいろいろな面(ウォレット、全てのブロックチェインのコピーを保持したトランザクション確認エンジン、peer-to-peer bitcoinネットワーク内でのノード)を実装しています。
...
((("bitcoin.org","Bitcoin Core, downloading"))) http://bitcoin.org/en/choose-your-wallet[Bitcoin's Choose Your Wallet page] で、bitcoinクライアントの実装からbitcoin coreを選びましょう。あなたが使っているOSによっては、実行可能なインストーラーがダウンロードできるでしょう。もしWindowsなら、ZIPアーカイブか.exe実行ファイルです。もしMac OSなら、.dmgディスクイメージ。Linuxなら、UbuntuのPPAパッケージかtar.gzアーカイブです。<<bitcoin-choose-client>>図にあるように、bitcin.orgには推薦されるbitcoinクライアントがリストアップされています。
...
平文のトランザクションには、トランザクションインプット/アウトプットを含む全ての項目が表示されます。この場合トランザクションには、さきほど作った新しいbitcoinアドレスへの50mbitsの送付に対応した１つのインプットとそれに対して生成された２つのアウトプットが含まれます。インプットは前に検証されたトランザクションのアウトプットだったもので、 +d3c7+ で始まるvinのtxidのところに書かれています。２つのアウトプットは50mbits分のクレジットと送付元に送り返されるおつりを示します。
...
このブロックには367個のトランザクションが含まれていて、見て分かるように18番目の9ca8f9から始まるトランザクションが、我々のbitcoinアドレスに50mbitsクレジットが送ったトランザクション txidです。 +height+ パラメータはこのブロックがブロックチェインの286384番目のブロックであることを示しています。
