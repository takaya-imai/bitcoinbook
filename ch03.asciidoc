...
((("libbitcoin library")))((("libraries, alternative","libbitcoin library")))((("sx tools")))libbitcoinライブラリはスケーラブルなマルチスレッド&モジュール型C++実装で、full-node(部分的ではなく全部のブロックチェーンを保持する)とsxと呼ばれるコマンドラインベースのツールセットをサポートしています。このツールセットは、この章で説明したbitcoindのコマンドと同じものを多くサポートしています。sxツールはbitcoindで提供されていない管理ツールや操作ツール、type-2 deterministic key、key mnemonicsも持っています。????
