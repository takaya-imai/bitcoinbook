...
P2SH redeem scriptの中にP2SHを置くことはできませんという点に注意してください。というのは、P2SHの設計で再帰ができないようになっているためです。また、 +OP_RETURN+ は定義によって回収されることができないので、まだredeem scriptの中で +OP_RETURN+ を使用することはできません。
