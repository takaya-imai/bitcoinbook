...
((("transactions","structure of")))トランザクションは、資金源( _インプット_ と呼ばれる)から送り先( _アウトプット_ と呼ばれる)への価値の転送をエンコードした ((("data structure"))) _構造化データ_ です。トランザクションインプットとアウトプットは銀行口座または識別子と関連付いているわけではありません。????代わりに、これらを所有者にだけがもっている秘密鍵でロックされているbitcoinの固まりとして考えるべきです。トランザクションは<<tx_data_structure>>に示すようないくつかのフィールドを含んでいます。
