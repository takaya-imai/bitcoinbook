...
Pythonライブラリ pycoin (http://github.com/richardkiss/pycoin) はBitcoinキーやトランザクションの操作をサポートしているPythonベースライブラリです。元々Richard Kissによって書かれメンテナンスされているライブラリです。このライブラリは非標準トランザクションを適切に扱うことができるscript言語さえもサポートしています。2014年6月現在、最新バージョンは0.42です。ここにある全ての例コードはこのバージョンをベースにしています。注意点として、このライブラリはまだ活発に開発されているため、より新しいバージョンではAPIやコマンドラインツールの挙動が変わっているかもしれません。
...
GPGと _/dev/random_ のデフォルトのエントロピー源を用いBIP32キーを作成します:
...
BIP32文字列 (サブキー 0/3 に対応) を１つ選び、うまく動作するかチェック:
