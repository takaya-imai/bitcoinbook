...
* インプットのいずれもhash=0, N=-1でないか(coinbase transactionはリレーされていくべきでない)
...
* それぞれのインプットに対して、もしインプットが参照しているアウトプットがcoinbase ouputだった場合、+COINBASE_MATURITY+ (100) 検証数を少なくとも持っているか
