...
採掘に成功したブロックの報酬はプールサーバのBitcoinアドレスに支払われます。個々のマイナーではありません。報酬の分配総額がある閾値に達したら、プールサーバは繰り返しマイナーのBitcoinアドレスに支払いを行います。典型的に、プールマイニングサービスを提供するために、プールサーバは報酬の一定パーセントを手数料として徴収します。
